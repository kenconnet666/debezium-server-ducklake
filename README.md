...
| CREATE TABLE | ⏳ 数据驱动 | ⏳ 数据驱动 | 建表延迟到**首批数据到达**——空表不会出现在湖里（镜像语义：无数据无表） |
| ADD COLUMN | ⏳ 数据驱动 | ⏳ 数据驱动 | 新列随**下一条数据事件**出现（加列后无写入则湖列暂不建），旧行该列 NULL |
| DROP COLUMN | ✅ 审计流(sql_drop) | ✅ ddl 正则 | `follow-drop-column=false` 时湖保留历史列 |
| RENAME COLUMN | ✅ 目录语义判定 | ✅ RENAME/CHANGE 正则 | 湖侧真 rename，数据无损（列存在性检查幂等） |
| ALTER COLUMN TYPE | ✅ 数据驱动三级 | ✅ 同 | 就地 ALTER → 整表 CAST 重写 → 重建+快照重灌兜底 |
| 列 DEFAULT/NOT NULL/CHECK 变更 | ➖ 不跟随 | ➖ 不跟随 | 湖表设计上不带约束（分析场景无需），数据值不受影响 |
| ADD PRIMARY KEY（存量表补主键） | ✅ 重建+blocking 快照重灌 | ✅ 同 | 旧行主键回填无 CDC 事件，必须重灌（详见运维节） |
| DROP 主键 | ⚠️ 降级告警 | ⚠️ 降级告警 | 掉主键后事件无 key → 湖侧自该时刻起 insert-only（告警提示）；湖历史行保留 |
| RENAME TABLE | ❌ 暂不跟随 | ✅ 湖表同步改名 | PG 表改名后：旧湖表冻结、新名首批数据另起新湖表（存量不迁移） |
| TRUNCATE | ❌ 暂不跟随 | ✅ 湖表清空（`follow-truncate` 可关） | PG 需要清空请用 `DELETE FROM` |
...
| DECIMAL / NUMERIC(p,s) | `DECIMAL(38,s)` | PG 裸 `numeric`（无精度）→ VARCHAR 保精度；⚠️ p>38 的超大值 CAST 溢出会阻塞批（见局限） |
...
| CHAR/VARCHAR/TEXT、ENUM、SET、UUID*、JSON | VARCHAR / UUID / JSON | *UUID 为 PG 特有；ENUM/SET 落文本 |
...
| PG 数组、hstore、自定义类型等 | VARCHAR 兜底 | toString 原文保全，不丢数据（数组暂不映射 DuckDB LIST） |
